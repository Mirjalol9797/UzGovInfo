/**
 * Инспекция по контролю за безопасностью объектов водного хозяйства и водопользованием при Минводхозе
 * Официальный раздел: https://gov.uz/oz/vnadzor
 */
export default {
  ru: {
    pageTitle:
      "Инспекция по контролю за безопасностью объектов водного хозяйства и водопользованием при Министерстве водного хозяйства Республики Узбекистан",
    intro: [
      "Инспекция по контролю за безопасностью объектов водного хозяйства и водопользованием при Министерстве водного хозяйства Республики Узбекистан осуществляет государственный надзор за безопасностью объектов водного хозяйства и соблюдением правил водопользования в пределах компетенции.",
      "На портале gov.uz в разделе vnadzor (https://gov.uz/ru/vnadzor) публикуются сведения об инспекции, организационной структуре, руководстве, центральном аппарате, подведомственных и региональных подразделениях, вакансиях. В блоках «Деятельность» размещаются материалы по стратегии развития, молодёжной политике, планам, антикоррупционной работе, исполнению законодательства, советам экспертов и общественности, международному сотрудничеству, гендерному равенству. В «Государственных услугах» — электронное правительство, вопросы и ответы, порядок работы. В «Документах» — Конституция, законы, акты Президента и Кабинета Министров, проекты НПА, нормативные и внутриведомственные акты инспекции, статистика, перечни публикуемой и не подлежащей разглашению информации. В «Цифровом правительстве» — данные по нормативным актам (в т.ч. № 3299, PF-6247), открытая бюджетная информация, социально значимые данные, реестр открытых данных, рассмотрение жалоб в сфере открытости, антикоррупция, методические руководства. Пресс-служба и контакты: новости, библиотека, выступления руководства, семинары, пресс-конференции, опросы, обратная связь, статистика обращений. Телефон доверия на портале: +998 55 520-71-66 (актуальные номера — в «Все номера» на gov.uz). Дополнительные материалы могут публиковаться на сайтах водного сектора.",
      "Полезен эксплуатирующим организациям водохозяйственных объектов, водопользователям, проектным и строительным организациям, а также гражданам при обращениях и поиске нормативных требований.",
    ],
    officialSite: {
      h2: "Официальный раздел на gov.uz",
      ctaText:
        "Перейти в раздел инспекции на gov.uz →",
    },
    whatFind: {
      h2: "Что можно найти на официальном ресурсе",
      intro:
        "Раздел gov.uz/vnadzor объединяет полномочия инспекции, перечень направлений деятельности и открытость.",
      listLabel: "На разделе обычно доступны:",
      items: [
        {
          icon: "building",
          title: "Об инспекции",
          desc: "О инспекции, структура, руководство, центральный аппарат, подведомственные и региональные органы, вакансии",
        },
        {
          icon: "program",
          title: "Деятельность",
          desc: "Стратегия и планы, антикоррупция, исполнение законодательства, советы, международное сотрудничество, гендерная политика",
        },
        {
          icon: "globe",
          title: "Государственные услуги",
          desc: "Электронное правительство, вопросы и ответы, порядок работы инспекции",
        },
        {
          icon: "decree",
          title: "Документы",
          desc: "Законы и подзаконные акты, проекты НПА, внутренние нормативные документы, статистика, перечни информации",
        },
        {
          icon: "chart",
          title: "Цифровое правительство",
          desc: "Открытый бюджет, открытые данные, жалобы по открытости, антикоррупционные материалы, методруководства",
        },
        {
          icon: "news",
          title: "Пресс-служба и контакты",
          desc: "Новости, библиотека, мероприятия, пресс-релизы, приём граждан, статистика обращений",
        },
      ],
      closing:
        "Меню обновляется; актуальные формы и требования смотрите на gov.uz/vnadzor.",
    },
    whyUse: {
      h2: "Зачем использовать официальный раздел",
      intro:
        "gov.uz/vnadzor — канал официальной информации инспекции на едином правительственном портале.",
      listLabel: "Раздел особенно полезен:",
      items: [
        "водохозяйственным организациям при подготовке к проверкам и исполнению требований",
        "водопользователям при уточнении правил и порядка обращений",
        "проектным и строительным организациям при работе с объектами водного хозяйства",
        "гражданам при подаче жалоб и поиске контактов региональных управлений",
      ],
    },
    otherSites: {
      h2: "Другие официальные сайты государственных органов",
    },
    schema: {
      description:
        "Официальный раздел инспекции по контролю за безопасностью объектов водного хозяйства и водопользованием при Министерстве водного хозяйства Республики Узбекистан на портале gov.uz (vnadzor).",
      orgName:
        "Инспекция по контролю за безопасностью объектов водного хозяйства и водопользованием при Министерстве водного хозяйства Республики Узбекистан",
      sameAs: [
        "https://gov.uz/ru/vnadzor",
        "https://gov.uz/en/vnadzor",
        "https://gov.uz/oz/vnadzor",
      ],
    },
    faq: {
      h2: "Часто задаваемые вопросы",
      items: [
        {
          q: "Где официальный раздел инспекции?",
          a: "На портале государства: https://gov.uz/ru/vnadzor (также https://gov.uz/en/vnadzor и https://gov.uz/oz/vnadzor). В каталоге сайта ссылка подставляется под язык интерфейса.",
        },
        {
          q: "Какой телефон доверия указан на портале?",
          a: "В блоке контактов обычно приводится +998 55 520-71-66; полный перечень — в «Все номера» на gov.uz.",
        },
        {
          q: "Где порядок работы инспекции и ответы на частые вопросы?",
          a: "В подразделах «Государственные услуги» на gov.uz/vnadzor: электронное правительство, вопросы и ответы, порядок работы.",
        },
        {
          q: "Где открытые данные и бюджетная информация инспекции?",
          a: "В блоке «Цифровое правительство» на разделе vnadzor (открытый бюджет, реестр открытых данных, социально значимая информация — уточняйте актуальные названия в меню).",
        },
        {
          q: "Кому подчиняется инспекция и где описана структура?",
          a: "Инспекция действует при Министерстве водного хозяйства Республики Узбекистан. Сведения об инспекции и структуре — в разделах «Об инспекции» и «Организационная структура» на https://gov.uz/ru/vnadzor.",
        },
      ],
    },
  },
  en: {
    pageTitle:
      "Inspection for control of safety of water management facilities and water use under the Ministry of Water Resources of the Republic of Uzbekistan",
    intro: [
      "The inspection body under the Ministry of Water Resources exercises state supervision over the safety of water management facilities and compliance with water use rules within its mandate.",
      "The gov.uz vnadzor section (https://gov.uz/en/vnadzor) publishes information on the inspection, structure, leadership, central office, subordinate and regional bodies, and vacancies. Activities cover development strategy, youth policy, plans, anti-corruption, legal compliance, expert and public councils, international cooperation and gender equality. Public services include e-government, Q&A and work procedures. Documents cover the Constitution, laws, presidential and government acts, draft regulations, the inspection’s normative and internal documents, statistics, and lists of published and restricted information. Digital government includes disclosures under relevant regulations (e.g. No. 3299, PF-6247), open budget data, socially significant information, open data registry, review of openness-related complaints, anti-corruption materials and guidelines. Press and contacts include news, a library, leadership statements, seminars, press events, surveys, feedback and appeal statistics. The portal hotline is commonly +998 55 520-71-66 (verify under “All numbers” on gov.uz). Additional materials may appear on sector websites.",
      "Useful for operators of water facilities, water users, design and construction organisations, and citizens filing appeals or seeking regulatory requirements.",
    ],
    officialSite: {
      h2: "Official section on gov.uz",
      ctaText: "Open the inspection section on gov.uz →",
    },
    whatFind: {
      h2: "What you can find there",
      intro:
        "gov.uz/vnadzor brings together the inspection’s mandate, activities and transparency.",
      listLabel: "You can typically find:",
      items: [
        {
          icon: "building",
          title: "About the inspection",
          desc: "Overview, structure, leadership, central staff, subordinate and regional bodies, vacancies",
        },
        {
          icon: "program",
          title: "Activities",
          desc: "Strategy and plans, anti-corruption, legal compliance, councils, international cooperation, gender policy",
        },
        {
          icon: "globe",
          title: "Public services",
          desc: "E-government, Q&A, work procedures",
        },
        {
          icon: "decree",
          title: "Documents",
          desc: "Laws and regulations, draft acts, internal documents, statistics, information lists",
        },
        {
          icon: "chart",
          title: "Digital government",
          desc: "Open budget, open data, openness complaints, anti-corruption, guidelines",
        },
        {
          icon: "news",
          title: "Press and contacts",
          desc: "News, library, events, press releases, citizen reception, appeal statistics",
        },
      ],
      closing: "Menus are updated; confirm current requirements on gov.uz/vnadzor.",
    },
    whyUse: {
      h2: "Why use this official resource",
      intro:
        "gov.uz/vnadzor is the inspection’s official channel on Uzbekistan’s government portal.",
      listLabel: "It is especially useful for:",
      items: [
        "water organisations preparing for inspections and meeting requirements",
        "water users clarifying rules and appeal procedures",
        "design and construction actors working on water facilities",
        "citizens filing complaints or finding regional contacts",
      ],
    },
    otherSites: {
      h2: "Other official websites of government bodies",
    },
    schema: {
      description:
        "Official section of the inspection for safety of water management facilities and water use under the Ministry of Water Resources of Uzbekistan on gov.uz (vnadzor).",
      orgName:
        "Inspection for control of safety of water management facilities and water use under the Ministry of Water Resources of the Republic of Uzbekistan",
      sameAs: [
        "https://gov.uz/en/vnadzor",
        "https://gov.uz/ru/vnadzor",
        "https://gov.uz/oz/vnadzor",
      ],
    },
    faq: {
      h2: "Frequently asked questions",
      items: [
        {
          q: "Where is the inspection’s official section?",
          a: "On the government portal: https://gov.uz/en/vnadzor (also https://gov.uz/ru/vnadzor and https://gov.uz/oz/vnadzor). The catalog link matches your interface language.",
        },
        {
          q: "What is the hotline on the portal?",
          a: "The portal usually lists +998 55 520-71-66; see “All numbers” on gov.uz for the full list.",
        },
        {
          q: "Where are work procedures and FAQs?",
          a: "Under Public services on gov.uz/vnadzor: e-government, Q&A, work procedures.",
        },
        {
          q: "Where are open data and budget disclosures?",
          a: "In the Digital government block on the vnadzor section (open budget, open data registry — check the live menu).",
        },
        {
          q: "Which ministry oversees the inspection, and where is the structure?",
          a: "The inspection operates under the Ministry of Water Resources of Uzbekistan. Overview and structure are under About the inspection and Organizational structure on https://gov.uz/en/vnadzor.",
        },
      ],
    },
  },
  uz: {
    pageTitle:
      "Oʻzbekiston Respublikasi Suv xoʻjaligi vazirligi huzuridagi Suv xoʻjaligi obyektlari xavfsizligini va suvdan foydalanishni nazorat qilish inspeksiyasi",
    intro: [
      "Oʻzbekiston Respublikasi Suv xoʻjaligi vazirligi huzuridagi Suv xoʻjaligi obyektlarining xavfsizligi va suvdan foydalanish qoidalariga rioya etilishini davlat nazorati ostida saqlash inspeksiyasi vakolat doirasida faoliyat yuritadi.",
      "gov.uz «vnadzor» boʻlimi (https://gov.uz/oz/vnadzor) da inspeksiya, tuzilma, rahbariyat, markaziy apparat, boʻysunuvchi va hududiy organlar, vakansiyalar haqida maʼlumotlar joylashtiriladi. «Faoliyat»da rivojlanish strategiyasi, yoshlar siyosati, rejalar, korrupsiyaga qarshi kurash, qonunchilik ijrosi, ekspert va jamoatchilik kengashlari, xalqaro hamkorlik, gender tengligi. «Davlat xizmatlari»da elektron hukumat, savollar-javoblar, ish tartibi. «Hujjatlar»da Konstitutsiya, qonunlar, Prezident va Vazirlar Mahkamasi hujjatlari, loyiha NPHlar, inspeksiyaning meʼyoriy va ichki hujjatlari, statistika, eʼlon qilinadigan va eʼlon qilinmaydigan maʼlumotlar roʻyxati. «Raqamli davlat»da normativ hujjatlar boʻyicha maʼlumotlar (jumladan 3299-son Nizom, PF-6247), ochiq byudjet, ijtimoiy ahamiyatli maʼlumotlar, ochiq maʼlumotlar reyestri, ochiqlik boʻyicha shikoyatlar, korrupsiyaga qarshi kurash, yoʻriqnomalar. Matbuot va kontaktlar: yangiliklar, kutubxona, rahbariyat nutqlari, seminarlar, matbuot anjumanlari, soʻrovnomalar, murojaatlar statistikasi. Ishonch telefoni: +998 55 520-71-66 («Barcha raqamlar» dan tekshiring). Qoʻshimcha materiallar suv sektori saytlarida boʻlishi mumkin.",
      "Suv obyektlarini ekspluatatsiya qiluvchi tashkilotlar, suv isteʼmolchilari, loyiha-qurilish tashkilotlari va murojaat qiluvchi fuqarolar uchun foydali.",
    ],
    officialSite: {
      h2: "gov.uz dagi rasmiy boʻlim",
      ctaText: "gov.uz da inspeksiya boʻlimiga oʻtish →",
    },
    whatFind: {
      h2: "Rasmiy resursda nimalar bor",
      intro: "gov.uz/vnadzor inspeksiya vakolati, yoʻnalishlar va ochiqlikni birlashtiradi.",
      listLabel: "Odatda mavjud:",
      items: [
        {
          icon: "building",
          title: "Inspeksiya haqida",
          desc: "Inspeksiya haqida, tashkilot tuzilmasi, rahbariyat, markaziy apparat, tizimdagi tashkilotlar, hududiy boshqarmalar, boʻsh ish oʻrinlari",
        },
        {
          icon: "program",
          title: "Faoliyat",
          desc: "Rivojlanish strategiyasi, yoshlar siyosati, rejalar, monopoliyaga qarshi komplayens, qonunchilik ijrosi, ekspert va jamoatchilik kengashlari, xalqaro hamkorlik, gender tengligi",
        },
        {
          icon: "globe",
          title: "Davlat xizmatlari",
          desc: "Elektron hukumat, savollar-javoblar, ish tartibi",
        },
        {
          icon: "decree",
          title: "Hujjatlar",
          desc: "Qonunlar va qarorlar, loyiha NPHlar, ichki hujjatlar, statistika, maʼlumotlar roʻyxati",
        },
        {
          icon: "chart",
          title: "Raqamli davlat",
          desc: "Ochiq byudjet, ochiq maʼlumotlar, ochiqlik boʻyicha shikoyatlar, yoʻriqnomalar",
        },
        {
          icon: "news",
          title: "Axborot xizmati va kontaktlar",
          desc: "Yangiliklar, kutubxona, tadbirlar, murojaatlar statistikasi",
        },
      ],
      closing: "Menyu yangilanadi; dolzarb talablarni gov.uz/vnadzor dan tekshiring.",
    },
    whyUse: {
      h2: "Nima uchun rasmiy boʻlimdan foydalanish kerak",
      intro:
        "gov.uz/vnadzor — yagona davlat portali boʻyicha inspeksiyaning rasmiy axborot kanali.",
      listLabel: "U ayniqsa quyidagilar uchun muhim:",
      items: [
        "tekshiruvlarga tayyorgarlik va talablarga rioya qiluvchi suv xoʻjaligi tashkilotlari uchun",
        "qoidalar va murojaat tartibini aniqlashtiruvchi suv isteʼmolchilari uchun",
        "suv obyektlari boʻyicha ishlovchi loyiha va qurilish tashkilotlari uchun",
        "murojaat va hududiy kontaktlarni qidiruvchi fuqarolar uchun",
      ],
    },
    otherSites: {
      h2: "Boshqa davlat organlarining rasmiy saytlari",
    },
    schema: {
      description:
        "Suv xoʻjaligi vazirligi huzuridagi Suv xoʻjaligi obyektlari xavfsizligi va suvdan foydalanishni nazorat qilish inspeksiyasining gov.uz portali boʻlimi (vnadzor).",
      orgName:
        "Oʻzbekiston Respublikasi Suv xoʻjaligi vazirligi huzuridagi Suv xoʻjaligi obyektlari xavfsizligini va suvdan foydalanishni nazorat qilish inspeksiyasi",
      sameAs: [
        "https://gov.uz/oz/vnadzor",
        "https://gov.uz/ru/vnadzor",
        "https://gov.uz/en/vnadzor",
      ],
    },
    faq: {
      h2: "Tez-tez beriladigan savollar",
      items: [
        {
          q: "Inspeksiyaning rasmiy boʻlimi qayerda?",
          a: "Davlat portali: https://gov.uz/oz/vnadzor (shuningdek https://gov.uz/ru/vnadzor va https://gov.uz/en/vnadzor). Katalogdagi havola interfeys tiliga mos keladi.",
        },
        {
          q: "Ishonch telefoni qanday?",
          a: "Odatda +998 55 520-71-66 koʻrsatiladi; toʻliq roʻyxat — gov.uz dagi «Barcha raqamlar».",
        },
        {
          q: "Ish tartibi va tez-tez so‘raladigan savollar qayerda?",
          a: "gov.uz/vnadzor dagi «Davlat xizmatlari» boʻlimida: elektron hukumat, savollar-javoblar, ish tartibi.",
        },
        {
          q: "Ochiq maʼlumotlar va byudjet axboroti qayerda?",
          a: "«Raqamli davlat» blokida, vnadzor boʻlimining joriy menyusidan tekshiring.",
        },
        {
          q: "Inspeksiya qaysi vazirlik huzurida va tuzilma qayerda?",
          a: "Suv xoʻjaligi vazirligi huzurida. Inspeksiya va tuzilma haqida — «Inspeksiya haqida» va «Tashkilot tuzilmasi» boʻlimlarida: https://gov.uz/oz/vnadzor.",
        },
      ],
    },
  },
};
