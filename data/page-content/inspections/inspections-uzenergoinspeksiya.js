/**
 * Инспекция по контролю за использованием электроэнергии, нефтепродуктов и газа при Кабмине
 * Официальный раздел: https://gov.uz/oz/uzenergoinspeksiya
 */
export default {
  ru: {
    pageTitle:
      "Инспекция по контролю за использованием электрической энергии, нефтепродуктов и газа при Кабинете Министров Республики Узбекистан",
    intro: [
      "Инспекция по контролю за использованием электрической энергии, нефтепродуктов и газа при Кабинете Министров Республики Узбекистан осуществляет государственный контроль в сфере электроэнергетики, оборота нефтепродуктов и использования газа в пределах компетенции.",
      "На портале gov.uz в разделе uzenergoinspeksiya (https://gov.uz/ru/uzenergoinspeksiya) публикуются сведения об инспекции, структуре, руководстве, центральном аппарате, региональных подразделениях. В «Деятельности» — обязательный энергоаудит, электронное правительство, международное сотрудничество, антикоррупционная работа, молодёжная и гендерная политика. В «Государственных услугах» — раздел вопросов и ответов. В «Документах» — законодательство, указы и постановления Президента, постановления и распоряжения правительства, зарегистрированные в Минюсте нормативные акты, Конституция. Предусмотрены блоки цифрового правительства и пресс-службы: новости, заявления руководства, объявления, пресс-релизы, пресс-конференции, опросы, обратная связь. Горячая линия на портале: +998 71 208-10-15 (актуальные номера — в «Все номера» на gov.uz).",
      "Полезен энергосбытовым и сетевым организациям, предприятиям ТЭК, проектным организациям и гражданам при обращениях и поиске нормативных требований.",
    ],
    officialSite: {
      h2: "Официальный раздел на gov.uz",
      ctaText:
        "Перейти в раздел энергетической инспекции на gov.uz →",
    },
    whatFind: {
      h2: "Что можно найти на официальном ресурсе",
      intro:
        "Раздел gov.uz/uzenergoinspeksiya объединяет полномочия инспекции и открытую информацию.",
      listLabel: "На разделе обычно доступны:",
      items: [
        {
          icon: "building",
          title: "Об инспекции",
          desc: "Об организации, структура инспекции, администрация, центральный аппарат, региональные управления",
        },
        {
          icon: "program",
          title: "Деятельность",
          desc: "Обязательный энергоаудит, электронное правительство, международное сотрудничество, антикоррупция, молодёжная и гендерная политика",
        },
        {
          icon: "globe",
          title: "Государственные услуги",
          desc: "Вопросы и ответы",
        },
        {
          icon: "decree",
          title: "Документы",
          desc: "Законы, акты Президента и правительства, зарегистрированные нормативные акты, Конституция",
        },
        {
          icon: "chart",
          title: "Цифровое правительство",
          desc: "Материалы раздела уточняйте в меню на портале",
        },
        {
          icon: "news",
          title: "Пресс-служба и контакты",
          desc: "Новости, выступления руководства, пресс-конференции, опросы, обратная связь",
        },
      ],
      closing:
        "Состав меню обновляется; актуальные требования смотрите на gov.uz/uzenergoinspeksiya.",
    },
    whyUse: {
      h2: "Зачем использовать официальный раздел",
      intro:
        "gov.uz/uzenergoinspeksiya — канал официальной информации инспекции на едином правительственном портале.",
      listLabel: "Раздел особенно полезен:",
      items: [
        "организациям при подготовке к энергоаудиту и соблюдению требований в сфере электроэнергии и топлива",
        "предприятиям ТЭК при работе с нормативными актами и обращениями",
        "проектным и эксплуатационным службам при поиске официальных разъяснений",
        "гражданам при обращениях и поиске контактов региональных подразделений",
      ],
    },
    otherSites: {
      h2: "Другие официальные сайты государственных органов",
    },
    schema: {
      description:
        "Официальный раздел Инспекции по контролю за использованием электрической энергии, нефтепродуктов и газа при Кабинете Министров Республики Узбекистан на портале gov.uz (uzenergoinspeksiya).",
      orgName:
        "Инспекция по контролю за использованием электрической энергии, нефтепродуктов и газа при Кабинете Министров Республики Узбекистан",
      sameAs: [
        "https://gov.uz/ru/uzenergoinspeksiya",
        "https://gov.uz/en/uzenergoinspeksiya",
        "https://gov.uz/oz/uzenergoinspeksiya",
      ],
    },
    faq: {
      h2: "Часто задаваемые вопросы",
      items: [
        {
          q: "Где официальный раздел инспекции?",
          a: "На портале государства: https://gov.uz/ru/uzenergoinspeksiya (также https://gov.uz/en/uzenergoinspeksiya и https://gov.uz/oz/uzenergoinspeksiya). В каталоге сайта ссылка подставляется под язык интерфейса.",
        },
        {
          q: "Какой телефон горячей линии указан на портале?",
          a: "Обычно приводится +998 71 208-10-15; полный перечень — в «Все номера» на gov.uz.",
        },
        {
          q: "Где сведения об обязательном энергоаудите?",
          a: "В блоке «Деятельность» на gov.uz/uzenergoinspeksiya.",
        },
        {
          q: "Где перечень нормативных документов?",
          a: "В разделе «Документы» на разделе uzenergoinspeksiya (законы, акты Президента и правительства, зарегистрированные акты, Конституция).",
        },
        {
          q: "Кому подчиняется инспекция и где описана структура?",
          a: "Инспекция действует при Кабинете Министров Республики Узбекистан. Сведения о структуре и руководстве — в разделах «Об организации» и «Структура инспекции» на https://gov.uz/ru/uzenergoinspeksiya.",
        },
      ],
    },
  },
  en: {
    pageTitle:
      "Inspection for Control in electric power industry, oil products, and gas usage at the Cabinet of Ministers of the Republic of Uzbekistan",
    intro: [
      "The inspection body under the Cabinet of Ministers exercises state control over the use of electricity, oil products and gas within its mandate.",
      "The gov.uz uzenergoinspeksiya section (https://gov.uz/en/uzenergoinspeksiya) publishes information on the inspection, structure, leadership, central office and regional departments. Activities cover mandatory energy audit, e-government, international cooperation, anti-corruption, youth and gender policy. Public services include FAQs. Documents cover legislation, presidential and government acts, regulations registered with the Ministry of Justice, and the Constitution. Digital government and press blocks include news, leadership statements, announcements, press releases, press conferences, surveys and feedback. The portal hotline is commonly +998 71 208-10-15 (verify under “All numbers” on gov.uz).",
      "Useful for power utilities and fuel sector enterprises, designers and citizens filing appeals or seeking regulatory information.",
    ],
    officialSite: {
      h2: "Official section on gov.uz",
      ctaText: "Open the energy inspection section on gov.uz →",
    },
    whatFind: {
      h2: "What you can find there",
      intro:
        "gov.uz/uzenergoinspeksiya brings together the inspection’s mandate and public information.",
      listLabel: "You can typically find:",
      items: [
        {
          icon: "building",
          title: "About the Inspection",
          desc: "About the organization, structure, administration, central staff, regional departments",
        },
        {
          icon: "program",
          title: "Activity",
          desc: "Mandatory energy audit, e-government, international cooperation, anti-corruption, youth and gender policy",
        },
        {
          icon: "globe",
          title: "Public services",
          desc: "FAQ",
        },
        {
          icon: "decree",
          title: "Documents",
          desc: "Laws, presidential and government acts, regulations registered with the Ministry of Justice, Constitution",
        },
        {
          icon: "chart",
          title: "Digital government",
          desc: "See the live menu on the portal for current items",
        },
        {
          icon: "news",
          title: "Press and contacts",
          desc: "News, leadership statements, press releases, press conferences, surveys, feedback",
        },
      ],
      closing: "Menus are updated; confirm current requirements on gov.uz/uzenergoinspeksiya.",
    },
    whyUse: {
      h2: "Why use this official resource",
      intro:
        "gov.uz/uzenergoinspeksiya is the inspection’s official channel on Uzbekistan’s government portal.",
      listLabel: "It is especially useful for:",
      items: [
        "organizations preparing for mandatory energy audits and compliance in power and fuels",
        "energy-sector enterprises working with regulations and appeals",
        "design and operations teams seeking official guidance",
        "citizens filing appeals or finding regional contacts",
      ],
    },
    otherSites: {
      h2: "Other official websites of government bodies",
    },
    schema: {
      description:
        "Official section of the Inspection for control in electric power industry, oil products, and gas usage at the Cabinet of Ministers of the Republic of Uzbekistan on gov.uz (uzenergoinspeksiya).",
      orgName:
        "Inspection for Control in electric power industry, oil products, and gas usage at the Cabinet of Ministers of the Republic of Uzbekistan",
      sameAs: [
        "https://gov.uz/en/uzenergoinspeksiya",
        "https://gov.uz/ru/uzenergoinspeksiya",
        "https://gov.uz/oz/uzenergoinspeksiya",
      ],
    },
    faq: {
      h2: "Frequently asked questions",
      items: [
        {
          q: "Where is the inspection’s official section?",
          a: "On the government portal: https://gov.uz/en/uzenergoinspeksiya (also https://gov.uz/ru/uzenergoinspeksiya and https://gov.uz/oz/uzenergoinspeksiya). The catalog link matches your interface language.",
        },
        {
          q: "What is the hotline on the portal?",
          a: "The portal usually lists +998 71 208-10-15; see “All numbers” on gov.uz for the full list.",
        },
        {
          q: "Where is information on mandatory energy audit?",
          a: "Under Activity on gov.uz/uzenergoinspeksiya.",
        },
        {
          q: "Where is the list of legal documents?",
          a: "Under Documents on the uzenergoinspeksiya section (laws, presidential and government acts, registered regulations, Constitution).",
        },
        {
          q: "Which body oversees the inspection, and where is the structure?",
          a: "The inspection operates under the Cabinet of Ministers of Uzbekistan. Structure and leadership are under About the organization and Structure of the inspection on https://gov.uz/en/uzenergoinspeksiya.",
        },
      ],
    },
  },
  uz: {
    pageTitle:
      "O'zbekiston Respublikasi Vazirlar Mahkamasi huzuridagi elektr energiyasi, neft mahsulotlari va gazdan foydalanishni nazorat qilish inspeksiyasi",
    intro: [
      "Oʻzbekiston Respublikasi Vazirlar Mahkamasi huzuridagi elektr energiyasi, neft mahsulotlari va gazdan foydalanishni nazorat qilish inspeksiyasi elektr energiya, neft mahsulotlari va gazdan foydalanish sohasida davlat nazoratini vakolat doirasida amalga oshiradi.",
      "gov.uz «uzenergoinspeksiya» boʻlimi (https://gov.uz/oz/uzenergoinspeksiya) da inspeksiya, tuzilma, rahbariyat, markaziy apparat, hududiy boshqarmalar haqida maʼlumotlar joylashtiriladi. «Faoliyat»da majburiy energoaudit, elektron hukumat, xalqaro hamkorlik, korrupsiyaga qarshi kurash, yoshlar va gender siyosati. «Davlat xizmatlari»da savollar-javoblar. «Hujjatlar»da qonunchilik, Prezident va hukumat qarorlari, Adliya vazirligi roʻyxatidan oʻtgan normativ hujjatlar, Konstitutsiya. «Raqamli davlat» va matbuot: yangiliklar, rahbariyat bayonotlari, eʼlonlar, press-relizlar, matbuot anjumanlari, soʻrovnomalar, qayta aloqa. Ishonch liniyasi: +998 71 208-10-15 («Barcha raqamlar» dan tekshiring).",
      "energetika va yoqilgʻi sektori, loyiha va fuqarolar uchun foydali.",
    ],
    officialSite: {
      h2: "gov.uz dagi rasmiy boʻlim",
      ctaText: "gov.uz da energiya inspeksiyasi boʻlimiga oʻtish →",
    },
    whatFind: {
      h2: "Rasmiy resursda nimalar bor",
      intro:
        "gov.uz/uzenergoinspeksiya inspeksiya vakolati va ochiq axborotni birlashtiradi.",
      listLabel: "Odatda mavjud:",
      items: [
        {
          icon: "building",
          title: "Inspeksiya haqida",
          desc: "Tashkilot haqida, inspeksiya tuzilmasi, boshqaruv, markaziy apparat, hududiy boʻlinmalar",
        },
        {
          icon: "program",
          title: "Faoliyat",
          desc: "Majburiy energoaudit, elektron hukumat, xalqaro hamkorlik, korrupsiyaga qarshi kurash",
        },
        {
          icon: "globe",
          title: "Davlat xizmatlari",
          desc: "Savollar-javoblar",
        },
        {
          icon: "decree",
          title: "Hujjatlar",
          desc: "Qonunlar, Prezident va hukumat hujjatlari, roʻyxatdan oʻtgan normativ hujjatlar, Konstitutsiya",
        },
        {
          icon: "chart",
          title: "Raqamli davlat",
          desc: "Joriy menyu boʻyicha portalda tekshiring",
        },
        {
          icon: "news",
          title: "Axborot xizmati va kontaktlar",
          desc: "Yangiliklar, rahbariyat nutqlari, matbuot anjumanlari, soʻrovnomalar",
        },
      ],
      closing:
        "Menyu yangilanadi; dolzarb talablarni gov.uz/uzenergoinspeksiya dan tekshiring.",
    },
    whyUse: {
      h2: "Nima uchun rasmiy boʻlimdan foydalanish kerak",
      intro:
        "gov.uz/uzenergoinspeksiya — yagona davlat portali boʻyicha inspeksiyaning rasmiy axborot kanali.",
      listLabel: "U ayniqsa quyidagilar uchun muhim:",
      items: [
        "majburiy energoaudit va talablarga rioya qiluvchi tashkilotlar uchun",
        "energetika va yoqilgʻi sohasidagi korxonalar uchun",
        "meʼyoriy izoh va murojaatlar uchun loyiha va ekspluatatsiya xizmatlari uchun",
        "murojaat va hududiy kontaktlarni qidiruvchi fuqarolar uchun",
      ],
    },
    otherSites: {
      h2: "Boshqa davlat organlarining rasmiy saytlari",
    },
    schema: {
      description:
        "Vazirlar Mahkamasi huzuridagi elektr energiyasi, neft mahsulotlari va gazdan foydalanishni nazorat qilish inspeksiyasining gov.uz portali boʻlimi (uzenergoinspeksiya).",
      orgName:
        "O'zbekiston Respublikasi Vazirlar Mahkamasi huzuridagi elektr energiyasi, neft mahsulotlari va gazdan foydalanishni nazorat qilish inspeksiyasi",
      sameAs: [
        "https://gov.uz/oz/uzenergoinspeksiya",
        "https://gov.uz/ru/uzenergoinspeksiya",
        "https://gov.uz/en/uzenergoinspeksiya",
      ],
    },
    faq: {
      h2: "Tez-tez beriladigan savollar",
      items: [
        {
          q: "Inspeksiyaning rasmiy boʻlimi qayerda?",
          a: "Davlat portali: https://gov.uz/oz/uzenergoinspeksiya (shuningdek https://gov.uz/ru/uzenergoinspeksiya va https://gov.uz/en/uzenergoinspeksiya). Katalogdagi havola interfeys tiliga mos keladi.",
        },
        {
          q: "Ishonch telefoni qanday?",
          a: "Odatda +998 71 208-10-15; toʻliq roʻyxat — gov.uz dagi «Barcha raqamlar».",
        },
        {
          q: "Majburiy energoaudit haqida maʼlumot qayerda?",
          a: "gov.uz/uzenergoinspeksiya dagi «Faoliyat» boʻlimida.",
        },
        {
          q: "Normativ hujjatlar roʻyxati qayerda?",
          a: "«Hujjatlar» boʻlimida: qonunlar, Prezident va hukumat hujjatlari, roʻyxatdan oʻtgan aktlar, Konstitutsiya.",
        },
        {
          q: "Inspeksiya qayerda tuzilgan va tuzilma qayerda?",
          a: "Oʻzbekiston Respublikasi Vazirlar Mahkamasi huzurida. Tuzilma — «Tashkilot haqida» va «Inspeksiya tuzilmasi» boʻlimlarida: https://gov.uz/oz/uzenergoinspeksiya.",
        },
      ],
    },
  },
};
